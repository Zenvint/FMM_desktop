...
const NewClassForm = () => {
    const [addNewClass, { isLoading, isSuccess, isError, error }] = useAddNewClassMutation();
    const {
        data: sections,
    ...
    const onSaveClassClicked = async (e) => {
      e.preventDefault();
      if (canSave) {
        await addNewClass({ classname: classname, sectionId: selectedSection, tuition: tuition});
      }
    };
    ...
    if (isSuccessSections){
        const { ids } = sections;
        const sectionsObject = ids?.length && ids.map((userId) => sections?.entities[userId]);
        sectionsListOptions = sectionsObject?.length && sectionsObject.map((sectionObj) => <option key={sectionObj.id} value={sectionObj.id} >{sectionObj.sectionname}</option>)
    }

    const errContent = (error?.data?.message || errorSection?.data?.message) ?? "";
      ...
      <Box m="20px">
        ...
        <form onSubmit={onSaveClassClicked}>
          ...
          <div className="input-container">
           ...
           {isSuccessSections && <div className="class_container">
			  <div className="input-container">
				<label htmlFor="sections" className="dropdown">
				  {" "}
				  Sections:{" "}
				</label>
				<select id="sections" placeholder="Select the section" value={selectedSection} onChange={handleSectionChange} required>
